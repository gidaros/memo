...
public class OpenIDLoginController extends AbstractLoginController {
	...
	/**
	 * Handles OpenID login verification. Determines whether the user is already
	 * a member (performs sign-in) or not (initiates sign-up). At any case the
	...
	private String getReturnTo(HttpServletRequest request) {
		...
		return realm + request.getContextPath() + RETURN_TO;
	}
}
